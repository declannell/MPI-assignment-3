...
void exchangrma_2d(double x[][maxn], int nx, int s[2], int e[2], MPI_Win win,
	...
	MPI_Type_vector(num_horz_elements, 1, maxn, MPI_DOUBLE,
					&row_type); // We have to skip maxn as this is actually the size of the grid, not nx + 2.
	MPI_Type_commit(&row_type);

	//between two processors.
	MPI_Aint offset;
	MPI_Win_fence(MPI_MODE_NOPRECEDE,
				  win);//We can do all of these within one win fence as all act on different parts of memory.
	...
	MPI_Get(&x[e[0] + 1][s[1]], num_vertical_elements, MPI_DOUBLE, nbrright, offset,
			num_vertical_elements, MPI_DOUBLE, win);
	...
	offset = s[1];
	MPI_Put(&x[e[0]][s[1]], num_vertical_elements, MPI_DOUBLE, nbrright, offset,
			num_vertical_elements, MPI_DOUBLE, win);

	offset = maxn + e[1];//We want to move 1 column in hence an offset of max n. We then want to place this e[1] up the column.
	MPI_Put(&x[s[0]][e[1]], 1, row_type, nbrup, offset, 1, row_type,
			win);//we make use of row_type and only pass 1 datatype.
...
void exchangrma_2d_pscw(double x[][maxn], int nx, int s[2], int e[2], MPI_Win win,
						int nbrleft, int nbrright, int nbrup, int nbrdown, int myid,
						MPI_Group edits[2], MPI_Group edited_by[2]) {
	int num_vertical_elements = e[1] - s[1] + 1, num_horz_elements = e[0] - s[0] + 1; //This is the number of points shared across the boundary
	...
	if (nbrdown != MPI_PROC_NULL) {//This means there is a process below  myid which will access the local memory on myid
		MPI_Win_post(edited_by[1], 0,
					 win);//This group is the process nbrdown which edits the window on myid
		MPI_Win_wait(win);
	}
	...
	if (nbrleft != MPI_PROC_NULL) {//This means it won't execute unless there is a process to myid left which will access the local memory on myid
		MPI_Win_post(edited_by[0], 0,
					 win);//This group is the processes, down and left each edit the window on myid
		MPI_Win_wait(win);
	}
