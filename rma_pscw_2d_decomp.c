...
void
gather_grid_2d(double a[][maxn], int nx, int size, int myid, int s[2], int e[2], MPI_Comm comm);

void
make_groups(MPI_Comm comm, MPI_Group edits[2], MPI_Group edited_by[2], int nbrright, int nbrleft,
			int nbrup, int nbrdown, int myid);
...
int main(int argc, char **argv) {
	...
	if (myid == 0) {
		printf(
			"The number of processors in the first dimension is %d. The number of processors in the second dimesion is %d\n",
			dim[0], dim[1]);
	}
	...
	printf(
		" myid is %d,(%d, %d), nbrup = %d, nbrdown = %d, nbrright = %d, nbrleft = %d,s = ( %d, %d), e = (%d,%d)\n",
	...
	MPI_Win_create(&a[s[0] - 1][0], (maxn) * (e[0] - s[0] + 3) * sizeof(double), sizeof(double),
				   MPI_INFO_NULL, MPI_COMM_WORLD, &wina);
	MPI_Win_create(&b[s[0] - 1][0], (maxn) * (e[0] - s[0] + 3) * sizeof(double), sizeof(double),
				   MPI_INFO_NULL, MPI_COMM_WORLD, &winb);
	glob_diff = 1000;

	MPI_Group edits[2], edited_by[2]; //this will store what process myid will edit and be editted by. The function make_groups does this.
	...
	for (it = 0; it < maxit; it++) {
		...
		exchangrma_2d_pscw(a, ny, s, e, wina, nbrleft, nbrright, nbrup, nbrdown, myid, edits,
						   edited_by);
		//print_in_order(a, MPI_COMM_WORLD, nx);
		...
		// update a using b
		exchangrma_2d_pscw(b, ny, s, e, winb, nbrleft, nbrright, nbrup, nbrdown, myid, edits,
						   edited_by);
		//print_in_order(b, MPI_COMM_WORLD, nx);
	...
	ldiff = griddiff_2d(analytic, a, nx, s,
						e);// I changed the griddiff function to return actually difference and not the difference squared
	...
	if (myid == 0) {
		...
		print_full_grid(a, nx);
		print_grid_to_file("rma_pscw_2D.txt", a ,nx, ny);
	}
	MPI_Finalize();
	return 0;
}
...
make_groups(MPI_Comm comm, MPI_Group edits[2], MPI_Group edited_by[2], int nbrright, int nbrleft,
	...
	if (nbrright != MPI_PROC_NULL) {//This ensures to the right of myid is not a boundary. Processors will access memory to the right.
		//printf("myid  is %d, my right neighbour is %d\n", myid, nbrright);
		int ranks[1] = {nbrright};
		MPI_Group_incl(all_procs, 1, ranks,
					   edits);//edits is already a pointer to the first element of arrays edits
	}

	if (nbrleft != MPI_PROC_NULL) { //The ensures to the left of myid is not a boundary. Processors will be accessed by the left.
		//printf("myid  is %d, my down  neighbour is %d\n", myid, nbrleft);
		int ranks[1] = {nbrleft};
		MPI_Group_incl(all_procs, 1, ranks,
					   edited_by);//edited_by is already a pointer to the first element of the array edtied_by
	}

	if (nbrup != MPI_PROC_NULL) {//This ensure above myid is not a boundary. Processors will access memory of the processor above.
		//printf("myid  is %d, my up neighbour is %d\n", myid, nbrup);
		int ranks[1] = {nbrup};
		MPI_Group_incl(all_procs, 1, ranks, &edits[1]);
	}

	if (nbrdown != MPI_PROC_NULL) { //The ensures below myid is not a  boundary. Processors are accessed from below.
		//printf("myid  is %d, my down  neighbour is %d\n", myid, nbrdown);
		int ranks[1] = {nbrdown};
		MPI_Group_incl(all_procs, 1, ranks, &edited_by[1]);
	}
...
gather_grid_2d(double a[][maxn], int nx, int size, int myid, int s[2], int e[2], MPI_Comm comm) {
	...
	if (myid == 0) {
		if (size > 16) {
			fprintf(stderr,
					"The number of processors is greater than 16 and hence the starting and end arrays are two small\n");
		}
	}
